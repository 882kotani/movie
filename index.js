...
app.post("/comp_conv", upload.single("file"), function (req, res, next) {
  ...
  // const o_filename = "out_sizedown_" + timestamp + "_" + i_filename;
  const o_filename = "sizedown_" + timestamp + "_" + i_filename;
  let command =
    // "ffmpeg -i public/uploads/" +
    "ffmpeg -i " +
    path_original_media +
    i_filename +
    " -b:v " +
...
app.post("/conv_conv", upload.single("file"), function (req, res, next) {
  ...
  const i_filename = req.file.originalname;
  let o_filename = "";
  if (req.body.conv_radio == "MOV_to_mp4") {
    o_filename = timestamp + ".mp4";
  } else {
    o_filename = timestamp + ".mp3";
  }
  const command =
    // "ffmpeg -i public/uploads/"
    "ffmpeg -i " +
...
app.post(
  ...
  function (req, res, next) {
    ...
    // res.send("完了！");
    res.download(path_conv_media + o_filename); // deleteFile(file1Paths);
